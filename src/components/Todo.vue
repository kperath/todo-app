    ...
    <div class="flex flex-col items-start">
      <div> <!-- div used to keep consistent width -->
          <div class="border-8 border-blue-900 mt-4 p-2 rounded-lg shadow-lg">
              ...
              <button @click="addItem" class="capitalize bg-red-500 text-red-900 ml-2 px-2 py-3 rounded-lg">Add Item</button>
          </div>
          <ul class="mt-2 rounded-lg">
              <li class="border-8 border-blue-800 bg-white p-2 mt-4 capitalize rounded-lg" v-for="(value, key) in items" :key="key">{{ value }}
                <!-- <button @click="removeItem(key)" class="float-right bg-red-500 text-red-900 lowercase px-2 ml-2 mb-2 rounded-sm">x</button> -->
                <button @click="removeItem(key)" class="float-right bg-red-500 text-red-900 lowercase px-2 ml-2 mb-2 rounded-full">
